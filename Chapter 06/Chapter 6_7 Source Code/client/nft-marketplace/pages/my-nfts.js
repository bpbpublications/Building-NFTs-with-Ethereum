...
export default function MyAssets() {
  ...
  async function loadNFTs() {
    ...
    console.log(`logging the value of data: ${JSON.stringify(data)}`)
    ...
    const nfts = await Promise.all(data.map(async i => {    
      try {
        ...
        console.log(`logging the value of meta: ${JSON.stringify(meta)}`)
        let nft = {
          price: i.price,
          tokenId: i.tokenId,
          seller: i.seller,
        ...
        console.log(`logging the value of nft: ${JSON.stringify(nft)}`)
        return nft
      } catch(err) {
        console.log(err)
        return null
